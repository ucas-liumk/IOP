<template>
  <section class="admin-page">
    <PageHeader title="日志" sub="平台操作日志 + 登录日志 · 跨全平台范围">
      <template #actions>
    ...
    <!-- Login-log limitation hint -->
    <div v-if="tab === 'login'" class="note">
      平台登录事件目前未持久化到 platform_audit_log（已知限制），此处仅显示动作含 "login" 的平台审计记录，通常为空。
    </div>

    <!-- Filters -->
    <div class="filters">
      <input class="input" v-model="filter.actor" placeholder="执行者 (platform_user id 前缀/全 id)" @keyup.enter="reload" />
      <input v-if="tab === 'oper'" class="input" v-model="filter.action" placeholder="动作 (如 platform.tenant_suspended)" @keyup.enter="reload" />
      ...
      <table class="data-table">
        ...
        <tbody>
          <tr v-for="a in entries" :key="a.id" @click="openDetail(a)" class="clickable">
            <td class="time">{{ fmt(a.occurred_at) }}</td>
    ...
    <!-- Pager (array endpoints have no total; infer next from a full page) -->
    <div class="pager">
...
import { listOperLogs, listLoginLogs, type PlatformLogEntry, type PlatformLogFilter } from "../api/system";

const PAGE_SIZE = 20;

const tab = ref<"oper" | "login">("oper");
const entries = ref<PlatformLogEntry[]>([]);
const detail = ref<PlatformLogEntry | null>(null);
const page = ref(1);
...
function buildFilter(): PlatformLogFilter {
  return {
...
function openDetail(a: PlatformLogEntry) { detail.value = a; }
...
function actorLabel(actor: string): string {
  ...
  return actor.length > 14 ? actor.slice(0, 10) + "…" : actor;
}
...
.note { font-size: 12px; color: var(--text-3); background: var(--surface-2); border: 1px solid var(--border-soft); padding: 8px 12px; border-radius: 8px; }

.filters { display: flex; gap: 8px; flex-wrap: wrap; align-items: center; }
.filters .input { flex: 0 1 240px; }
