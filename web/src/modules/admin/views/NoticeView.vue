<template>
  <section class="admin-page">
    <PageHeader title="通知公告" :sub="`共 ${notices.length} 条 · 草稿/已发布`">
      <template #actions>
        ...
        <button class="btn btn-primary" v-perm="'notice:write'" @click="openCreate">+ 新建公告</button>
      </template>
    </PageHeader>
    ...
    <DataTable :columns="columns" :rows="notices" row-key="id" empty-text="暂无公告 · 点击右上角新建">
      ...
      <template #cell-status="{ row }">
        <span class="badge" :class="row.status === 'published' ? 'on' : 'off'">
      ...
      <template #cell-actions="{ row }">
        <div class="row-actions">
          ...
          <button class="link-btn" v-perm="'notice:write'" @click="openEdit(row)">编辑</button>
          <button
            v-if="row.status !== 'published'"
            class="link-btn ok" v-perm="'notice:write'" @click="doPublish(row)"
          >发布</button>
          <button
            v-else
            class="link-btn warn" v-perm="'notice:write'" @click="doWithdraw(row)"
          >撤回</button>
          <button class="link-btn danger" v-perm="'notice:write'" @click="doDelete(row)">删除</button>
        </div>
      </template>
    </DataTable>
    ...
    <div v-if="editor" class="modal-overlay" @click.self="editor = null">
      <div class="modal modal-lg">
        ...
        <div class="modal-actions">
          <button class="btn btn-ghost" @click="editor = null">取消</button>
    ...
    <div v-if="viewing" class="modal-overlay" @click.self="viewing = null">
      <div class="modal modal-lg">
        <div class="view-head">
          <h3>{{ viewing.title }}</h3>
          <span class="badge" :class="viewing.status === 'published' ? 'on' : 'off'">
            {{ viewing.status === 'published' ? '已发布' : '草稿' }}
          </span>
        </div>
        <p class="view-meta">{{ typeLabel(viewing.type) }} · {{ viewing.created_at }}</p>
...
const editor = ref<Notice | true | null>(null);  // true = create, Notice = edit
...
function openView(n: Notice) { viewing.value = n; }
