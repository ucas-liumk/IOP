<template>
  <section class="admin-page">
    <PageHeader title="角色管理" :sub="`共 ${roles.length} 个角色 · 菜单权限 + 数据范围`">
      <template #actions>
    ...
    <div v-if="creating" class="modal-overlay" @click.self="creating = false">
      <div class="modal">
        ...
        <label class="field">
          ...
          <input class="input" v-model="form.code" pattern="[a-z_]+" placeholder="例如：editor" />
          <span class="hint">小写字母与下划线</span>
        </label>
        <label class="field">
          <span class="label">显示名称 *</span>
          <input class="input" v-model="form.name" placeholder="例如：编辑者" />
        </label>
...
import {
  ...
  type Role, type MenuNode, type Dept,
} from "../api/admin";
...
async function create() {
  ...
  try {
    const r = await createRole({ code: form.code.trim(), name: form.name.trim() });
